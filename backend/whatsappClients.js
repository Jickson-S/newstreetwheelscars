...
function getAllBotStatus() {
  const cachePath = path.join(__dirname, 'puppeteer_cache');
  return {
    ...
    diagnostics: {
      ...
      cacheContents: fs.existsSync(cachePath) ? listCacheFiles(cachePath) : null,
      envCacheDir: process.env.PUPPETEER_CACHE_DIR,
      envExecPath: process.env.PUPPETEER_EXECUTABLE_PATH
    }
  };
}
